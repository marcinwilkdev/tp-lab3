...
public abstract class CompositeShape extends Shape {
	...
	public boolean removeShape(Shape shape) {
		int oldLength = this.shapes.size();

		this.shapes = this.shapes.stream().filter(s -> !s.equals(shape)).collect(Collectors.toList());

		int newLength = this.shapes.size();
	...
	public int getShapeCount() {
		...
		for(Shape shape : this.shapes) {
			CompositeShape compositeShape = shape.asComposite();

			if(compositeShape == null) total += 1;
			else total += compositeShape.getShapeCount() + 1;
		}
	...
	public List<Shape> getShapesByType(ShapeType circle) {
		return this.shapes.stream().filter(s -> s.getType() == circle).collect(Collectors.toList());
	}
	...
	public List<Shape> getLeafShapes() {
		return this.shapes.stream().filter(s -> s.asComposite() == null).collect(Collectors.toList());
	}
